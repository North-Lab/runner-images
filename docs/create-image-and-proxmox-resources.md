...
1. **Template placement** — ensure a template replica exists on every target node. Reuses `ubuntu-2604-runner-<node>` when present. Shared storage skips this. After a new Packer build, `deploy --recreate-templates` (or `[proxmox].recreate_templates = true`, default off) **does not reuse** those replicas: it deletes `{template_name}-{node}` templates only, then copies the current source template to each node again (parallel). It refuses to delete a VM that is not a template, whose name is the source Packer template (no `-{node}` suffix), or that looks like a runner (`name_prefix`). Runner VMs and the source template stay in place.
