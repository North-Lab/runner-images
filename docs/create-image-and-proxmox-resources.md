...
| Node-local (`local-lvm`, ZFS, typical home lab) | **Phase A — template placement:** if `ubuntu-2604-runner-<node>` (or another matching template) is already on that node, reuse it unless `--recreate-templates` / `[proxmox].recreate_templates` is set. Otherwise (or after those replicas are deleted) clone the Packer template on the source node, **offline migrate** that replica with `with-local-disks=1`, and convert it to a template. Replica copies for missing nodes run in parallel. **Phase B — parallel clones:** full-clone every new runner VM from the **local** replica on its assigned node (all clones at once). **Phase C — boot/register:** start + wait for qemu-guest-agent (parallel), then guest-setup (sequential). `--recreate-templates` waits for each Proxmox delete UPID and polls until that replica is absent before copying the source again. |
...
1. **Template placement** — ensure a template replica exists on every target node. Reuses `ubuntu-2604-runner-<node>` when present. Shared storage skips this. After a new Packer build, `deploy --recreate-templates` (or `[proxmox].recreate_templates = true`, default off) **does not reuse** those replicas: it deletes `{template_name}-{node}` templates only, waits for each Proxmox delete UPID, polls until the replica is absent from inventory, and does not reuse those VMIDs/names in the same run (a failed delete is fatal). Then it copies the current source template to each node again (parallel). It refuses to delete a VM that is not a template, whose name is the source Packer template (no `-{node}` suffix), or that looks like a runner (`name_prefix`). Runner VMs and the source template stay in place.
...
Stops and deletes VMs whose names start with `name_prefix` (default `gh-runner-`) and removes matching GitHub runner registrations. `--count N` limits that to `gh-runner-01` … `N`. `--keep-github` leaves GitHub registrations. Packer templates (including per-node replicas named `ubuntu-2604-runner-<node>`) are not deleted. To replace those replicas after a new Packer build, use `deploy --recreate-templates` — not `destroy`.
